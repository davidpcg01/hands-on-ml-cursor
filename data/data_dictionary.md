...
### Columns
- `well_id`: completion label used in the repo
...
- `subset_start_date`: first month kept for the subset
- `subset_end_date`: last month kept for the subset
...
- The exercise uses a `36-month` modeling window per completion, split into `30` training months and `6` internal test months.
...
- The generated forecast plots should show daily rates derived from `days_on_prod`.
